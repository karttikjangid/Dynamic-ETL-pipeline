...
def map_pyarrow_to_app_type(type_str: str) -> str:
    ...
    Maps PyArrow type strings to standardized application types.
    This function is used when working with PyArrow schemas to
    normalize type names to our application's type system.
    
    Args:
        type_str: PyArrow type string (e.g., "int64", "double", "string")
        
    Returns:
        Standardized application type string
        
    Examples:
        >>> map_pyarrow_to_app_type("int64")
        'integer'
        >>> map_pyarrow_to_app_type("double")
        'number'
        >>> map_pyarrow_to_app_type("string")
        'string'
    """
    ...
    # Look up in mapping, default to "unknown"
...
def get_type_mapping() -> Dict[str, str]:
    ...
    Returns a dictionary mapping PyArrow type names to application
    type names. This ensures consistent type naming across the pipeline.
    
    Returns:
        Dict mapping PyArrow types to app types
        
    PyArrow Types → App Types:
    ...
    return {
        ...
        # Date/time types (kept as string for now)
        "date32": "string",
        "date64": "string",
        "timestamp": "string",
        "time32": "string",
